#!/usr/bin/env python3
"""
DEPRECATED: Use scheduler_daily.py instead.
...
No RTH check – runs once per day (cron ~17:00 EST) or on demand.
Only 1day and 1week (not 15min, 1h, 4h) for earnings assets.
...
def fetch_earnings_daily_weekly(symbol: str, provider_symbol: str) -> Tuple[int, Optional[str]]:
    """
    Fetch 1day + 1week bars for earnings tier. Appends to Parquet via BarsProvider.
...
def main():
    ...
    print(f"Earnings daily run – processing {len(daily_list)} symbols (canonical compute → compute.db)")
