...
Outputs:
- validation_outputs/era_step4_overlap_bias_summary.csv
...
# ===== ERA FILTERING (Step A / Step B) =====
# Set MODE to:
# "FULL"  -> no filter (1993-2026)
# "A2010" -> Version A: 2010-01-01 .. 2026-12-31
# "B_PRE2010" -> Version B1: 1993-01-01 .. 2009-12-31
# "B_2010_2019" -> Version B2: 2010-01-01 .. 2019-12-31
# "B_2020P" -> Version B3: 2020-01-01 .. 2026-12-31
MODE = "B_2020P"

ERA_RANGES = {
    "FULL": (None, None),
    "A2010": ("2010-01-01", "2026-12-31"),
    "B_PRE2010": ("1993-01-01", "2009-12-31"),
    "B_2010_2019": ("2010-01-01", "2019-12-31"),
    "B_2020P": ("2020-01-01", "2026-12-31"),
}
...
def main():
    ...
    df = load_joined()
    start, end = ERA_RANGES[MODE]
    if start is not None:
        df = df[df["ts"] >= start]
    if end is not None:
        df = df[df["ts"] <= end]
    print(f"[ERA] MODE={MODE}  rows_after_filter={len(df)}  ts_range={df['ts'].min()} -> {df['ts'].max()}")
    ...
    out_eps = Path("validation_outputs/era_step4_event_driven_episodes.csv")
    ...
    out_path = os.path.join(OUTPUT_DIR, "era_step4_overlap_bias_summary.csv")
